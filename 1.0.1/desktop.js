(function () {
  ...
  function setPreviewUi(state, text, url) {
    ...
    if (empty && state !== 'on') empty.textContent = text || '进入工程后会在这里播放';
    if (frame) {
      if (state === 'on' && url) {
  ...
  async function startPreview(project) {
    ...
    setPreviewUi('busy', '正在启动轻量播放器，全库只跑这一个…');
    try {
      ...
      previewPath = project.path;
      setPreviewUi('on', (info.reused ? '已接上 · ' : '已启动 · ') + (project.name || ''), info.url);
    } catch (err) {
      if (token !== previewToken) return;
      previewPath = '';
      setPreviewUi('err', '预览失败：' + ((err && err.message) || err));
    }
  }
  ...
  async function stopPreview(notify) {
    previewToken += 1;
    previewPath = '';
    setPreviewUi('off', '已停止。回到工程页会再启动。');
    try { await api.previewStop(); } catch (_) {}
    if (notify) toast('已停止预览');
  }
  ...
  async function boot() {
    ...
    if (live) live.textContent = 'Framespace 1.0.1';
    await scanNow('boot');
    refreshDoctor();
    refreshOccupancy();
