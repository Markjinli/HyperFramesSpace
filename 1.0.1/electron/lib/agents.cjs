...
function openAgent(agent, project) {
  if (!project || !project.path) throw new Error('先选一个工程');
  const cwd = String(project.path);
  if (!exists(cwd)) throw new Error('工程目录不存在：' + cwd);
  const a = String(agent || '').toLowerCase().replace(/\s+/g, '-');

  if (a === 'cursor') {
    const exe = resolveCursor();
    if (!exe) throw new Error('找不到 Cursor.exe');
  ...
  if (a === 'codex' || a === 'codex-cli') {
    const bin = resolveCodexJs();
    const node = resolveNode();
    if (!bin) throw new Error('找不到 Codex CLI');
    if (bin.endsWith('.js')) {
  ...
  if (a === 'grok' || a === 'grok-build' || a === 'grokbuild') {
    ...
    if (!exe) throw new Error('找不到 Grok Build：' + path.join(home(), '.grok', 'bin', 'grok.exe'));
  ...
  if (a === 'claude' || a === 'claude-code' || a === 'claudecode') {
    ...
    if (!exe) throw new Error('找不到 Claude Code（claude.exe）');
