...
function tr(locale, zh, en) { return locale === 'en' ? en : zh; }

async function inspect(projects, locale) {
  ...
  const disks = tr(locale, os.cpus().length + ' 线程 · ' + Math.round(os.totalmem() / 1073741824) + ' GB 内存', os.cpus().length + ' threads · ' + Math.round(os.totalmem() / 1073741824) + ' GB RAM');
  return {
    latestPin: latest,
    cards: [
      { title: 'Node.js', ok: /^v(2[2-9]|[3-9])/.test(node.text), detail: node.text || tr(locale, '未找到', 'Not found') },
      { title: 'HyperFrames CLI', ok: cli.ok, detail: latest ? ('latest ' + latest) : (cli.text.slice(0, 80) || tr(locale, 'npx 不可用', 'npx unavailable')) },
      { title: 'FFmpeg / FFprobe', ok: ffmpeg.ok && ffprobe.ok, detail: ffmpeg.ok ? tr(locale, '已找到', 'Found') : tr(locale, '未找到 ffmpeg', 'ffmpeg not found') },
      { title: 'npx', ok: npx.ok, detail: npx.ok ? tr(locale, '可用', 'Available') : tr(locale, '需要 Node.js / npm', 'Needs Node.js / npm') },
      { title: tr(locale, '工程 pin 漂移', 'Project pin drift'), ok: stale === 0, warn: stale > 0, detail: stale ? tr(locale, stale + ' 个落后', stale + ' behind') : tr(locale, '与 latest 对齐', 'Aligned with latest') },
      { title: tr(locale, '本机', 'This PC'), ok: true, detail: disks }
    ]
  };
}
