...
用 HyperFrames 做 vibe coding 视频时，工程会很快散落在磁盘各处。变体一多，只能靠文件夹名字记；要改片还得自己 `cd` 进目录，再开 Grok / Codex / Claude，再跑 `preview` / `check` / `render`。HyperFramesSpace 就是为这件事做的免费开源工作台：把本机 HyperFrames 工程收成库，点进去就能预览、抽帧、出片，并在正确的工程根打开 agent。
...
输出在 `dist/HyperFramesSpace.exe`。给 tag `v1.0.x` 推上去后，GitHub Actions 也会自动打 exe 并挂到 Release。
