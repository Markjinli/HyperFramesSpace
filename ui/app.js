(function (global) {
  ...
  function t(key, vars) {
    var api = global.FramespaceI18n;
    return api && typeof api.t === 'function' ? api.t(key, vars) : key;
  }
  function applyI18n() {
    if (global.FramespaceI18n && typeof global.FramespaceI18n.apply === 'function') {
      global.FramespaceI18n.apply(document);
    }
  }
  ...
  function hoverHideDelayMs() {
    var n = ui && ui.state ? Number(ui.state.hoverHideMs) : 1000;
  ...
  function placeHoverCard(rect, size, viewport) {
    ...
    if (top < 48) top = 48;
    var bridgeTop = Math.min(rect.top, top) - pad;
    var bridgeBottom = Math.max(rect.bottom, top + h) + pad;
    var bridge;
    if (side === 'right') {
      bridge = {
        left: Math.min(rect.right, left) - pad,
        top: bridgeTop,
        width: Math.max(24, Math.abs(left - rect.right) + pad * 2),
        height: Math.max(24, bridgeBottom - bridgeTop)
      };
    } else {
      bridge = {
        left: Math.min(rect.left, left + w) - pad,
        top: bridgeTop,
        width: Math.max(24, Math.abs(rect.left - (left + w)) + pad * 2),
        height: Math.max(24, bridgeBottom - bridgeTop)
      };
    }
  ...
  function virtualGridPlan(count, viewport, cardMin, overscan) {
    count = Number(count) || 0;
    overscan = overscan == null ? 2 : Number(overscan);
    var width = Math.max(1, Number(viewport && viewport.width) || 800);
    var height = Math.max(1, Number(viewport && viewport.height) || 600);
    var scrollTop = Math.max(0, Number(viewport && viewport.scrollTop) || 0);
    var gap = 14;
    var min = Number(cardMin) || 220;
    var cols = Math.max(1, Math.floor((width + gap) / (min + gap)));
    var cardW = Math.max(min, Math.floor((width - gap * (cols - 1)) / cols));
    var cardH = Math.round(cardW * 9 / 16) + 92;
    var stride = cardH + gap;
    var rows = Math.ceil(count / cols) || 0;
    var totalHeight = rows ? rows * stride - gap : 0;
    var startRow = Math.max(0, Math.floor(scrollTop / stride) - overscan);
    var endRow = Math.min(Math.max(0, rows - 1), Math.ceil((scrollTop + height) / stride) + overscan);
    return {
      cols: cols,
      gap: gap,
      cardW: cardW,
  ...
  function virtualListPlan(count, viewport, rowH, overscan) {
    count = Number(count) || 0;
    rowH = Number(rowH) || 64;
    overscan = overscan == null ? 4 : Number(overscan);
    var height = Math.max(1, Number(viewport && viewport.height) || 600);
    var scrollTop = Math.max(0, Number(viewport && viewport.scrollTop) || 0);
    var start = Math.max(0, Math.floor(scrollTop / rowH) - overscan);
    var end = Math.min(count, Math.ceil((scrollTop + height) / rowH) + overscan);
    return { rowH: rowH, totalHeight: count * rowH, start: start, end: end };
  }
  ...
  function mergeProject(detail) {
    ...
    for (var i = 0; i < CATALOG.length; i++) {
      if (CATALOG[i].id === detail.id) {
        CATALOG[i] = Object.assign({}, CATALOG[i], detail);
        return CATALOG[i];
      }
    }
    CATALOG.push(detail);
  ...
  function statusLabel(s) {
    return ({
      ...
      findings: t('status.findings'),
      rendered: t('status.rendered'),
      'stale-pin': t('status.stale'),
  ...
  function renderLibrary() {
    ...
    if (count) {
      ...
      count.textContent = t('dyn.projectsCtx', { n: list.length, ctx: ctx });
    }
    ...
    if (grid) {
      if (!list.length) {
        grid.style.height = '';
        grid.innerHTML = '<div class="empty">' + t('dyn.noMatch') + '</div>';
      } else {
        var gw = gridEl ? gridEl.clientWidth - 32 : 900;
        var gh = gridEl ? gridEl.clientHeight : 640;
        var plan = virtualGridPlan(list.length, {
          width: Math.max(200, gw),
          height: Math.max(200, gh),
          scrollTop: gridEl ? gridEl.scrollTop : 0
        }, cardMinPx(ui.state.cardSize));
        grid.classList.add('virtual-list');
        grid.style.height = plan.totalHeight + 'px';
        var html = '';
        for (var i = plan.start; i < plan.end; i++) {
          var p = list[i];
          ...
          var left = col * (plan.cardW + plan.gap);
          var top = row * plan.stride;
          html += (
            '<button class="card' + sel + '" type="button" data-open-project="' + p.id + '" data-hover-project="' + p.id + '"' + drag + ' style="left:' + left + 'px;top:' + top + 'px;width:' + plan.cardW + 'px">' +
              coverHtml(p, layout) +
              '<div class="card-body">' +
                ...
                '<div class="meta">' + p.duration + 's · ' + p.aspect + ' · ' + t('dyn.frames9') + '</div>' +
    ...
    if (rows) {
      if (!list.length) {
        ...
        var lh = listEl ? listEl.clientHeight : 640;
        var lplan = virtualListPlan(list.length, {
          height: Math.max(200, lh),
          scrollTop: listEl ? listEl.scrollTop : 0
        }, 64);
        rows.classList.add('virtual-list');
        rows.style.height = lplan.totalHeight + 'px';
        var rowHtml = '';
        for (var r = lplan.start; r < lplan.end; r++) {
          var rp = list[r];
          var rdrag = canDragProjects() ? ' draggable="true" data-drag-project="' + rp.id + '"' : '';
          rowHtml += (
            '<button class="row" type="button" data-open-project="' + rp.id + '" data-hover-project="' + rp.id + '"' + rdrag + ' style="top:' + (r * lplan.rowH) + 'px;height:' + lplan.rowH + 'px">' +
              coverHtml(rp, layout, 'mini-cover') +
              '<div><b>' + escapeHtml(rp.name) + '</b><div class="meta">' + escapeHtml(rp.brief) + '</div></div>' +
              '<div>' + rp.workflow + '</div>' +
              '<div>' + rp.duration + 's</div>' +
              '<div>' + rp.pin + '</div>' +
              '<div>' + statusLabel(rp.status) + '</div>' +
              '<div class="path">' + escapeHtml(rp.path) + '</div>' +
            '</button>'
          );
        }
    ...
    var listPane = $('#library-list');
    if (gridEl && listPane) {
      if (ui.state.layout === 'list') {
        gridEl.classList.add('is-off');
        listPane.classList.add('is-on');
      } else {
        gridEl.classList.remove('is-off');
        listPane.classList.remove('is-on');
      }
    }
  ...
  function renderPinned() {
    ...
    host.style.display = '';
    host.innerHTML = '<h4>' + t('dyn.pinnedFolders') + '</h4>' + pins.map(function (pin) {
      ...
      return (
        '<div class="pinned-card' + on + '">' +
          '<button class="series-card' + on + '" type="button" data-folder="' + escapeHtml(pin.path) + '"' + hover + ' title="' + escapeHtml(pin.path) + '">' +
            ...
            '<div class="series-meta"><b>' + escapeHtml(label) + '</b><span>' + t('dyn.folderProjects', { n: members.length }) + '</span></div>' +
          '</button>' +
          '<div class="pin-actions">' +
            '<button type="button" class="rename-btn" data-rename-pin="' + escapeHtml(pin.path) + '" title="' + t('dyn.renameTitle') + '">' + t('dyn.rename') + '</button>' +
            '<button type="button" class="pin-btn is-on" data-pin-folder="' + escapeHtml(pin.path) + '" title="' + t('dyn.unpin') + '">📌</button>' +
          '</div>' +
        '</div>'
      );
    }).join('');
  ...
  function renderCustomCollections() {
    ...
    var items = [
      { id: 'all', name: t('all'), count: CATALOG.length, mosaic: allCells, hoverId: CATALOG[0] && CATALOG[0].id, drop: false }
    ...
    host.innerHTML = items.map(function (it) {
      ...
      return (
        '<button class="series-card' + on + '" type="button" data-collection="' + escapeHtml(it.id) + '"' + hover + drop + '>' +
          it.mosaic +
          '<div class="series-meta"><b>' + escapeHtml(it.name) + '</b><span>' + t('dyn.projectsN', { n: it.count }) + (it.drop ? t('dyn.dropOk') : '') + '</span></div>' +
        '</button>'
      );
    }).join('');
  }
      ...
      return (
        '<div class="folder-node" style="--d:' + depth + '">' +
          '<div class="folder-row' + on + '">' +
            ...
            '<button type="button" class="pin-btn' + (pinned ? ' is-on' : '') + '" data-pin-folder="' + escapeHtml(node.path) + '" title="' + (pinned ? t('dyn.unpin') : t('dyn.pinLeft')) + '">' + (pinned ? '📌' : '📍') + '</button>' +
          '</div>' + kids +
        '</div>'
      );
    }
  ...
  function renderChrome() {
    ...
    if (hoverHide) hoverHide.value = String(ui.state.hoverHideMs == null ? 1000 : ui.state.hoverHideMs);
  ...
  function renderOccupancy() {
    ...
    setText('#chip-mem', t('dyn.memPct', { n: load.mem }));
    setText('#chip-gpu', load.gpuKnown === false ? t('dyn.gpuUnknown') : t('dyn.gpuPct', { n: load.gpu }));
    var kicker = load.rendering
      ? t('dyn.loadRendering', { p: load.processCount, o: load.orphanCount })
      : (load.paused
        ? t('dyn.loadPaused', { p: load.processCount })
        : (load.processCount ? t('dyn.loadBusy', { p: load.processCount, o: load.orphanCount }) : t('dyn.idle')));
    setText('#load-kicker', kicker);
    setText('#status-proc', t('dyn.statusTriple', { cpu: load.cpu, mem: load.mem, gpu: load.gpu }));
    var pauseLabel = load.paused && !load.rendering ? t('chip.resume') : t('chip.pause');
  ...
  function renderProcesses() {
    ...
    setText('#proc-summary', t('dyn.procSummary', { n: list.length, o: orphans.length }));
    if (!list.length) {
      host.innerHTML = '<div class="empty">' + t('dyn.noProc') + '</div>';
      return;
    }
    renderOccupancy();
    host.innerHTML = list.map(function (p) {
      return (
        '<div class="proc' + (p.orphan ? ' is-orphan' : '') + (p.paused ? ' is-paused' : '') + '">' +
          '<div><b>' + escapeHtml(p.name) + ' · ' + escapeHtml(p.kind) + (p.paused ? ' · ' + t('dyn.paused') : '') + '</b>' +
            '<div class="meta">' + escapeHtml(p.cmd) + '</div></div>' +
          '<div class="meta">' + escapeHtml(p.project || '—') + '</div>' +
          '<div class="meta">pid ' + p.pid + (p.port ? ' · :' + p.port : '') + '</div>' +
          '<div class="meta">' + (p.orphan ? t('dyn.orphan') + ' ' : '') + p.ageMin + ' min</div>' +
          '<button class="btn" type="button" data-kill-pid="' + p.pid + '">' + t('dyn.kill') + '</button>' +
        '</div>'
      );
    }).join('');
  }
  ...
  function renderSkills() {
    ...
    if (ui.state.skillTab === 'hot') {
      ...
      setText('#skill-count', t('dyn.hotCount', { n: merged.length, state: ui.state.lastCatalogAt ? t('dyn.checked') : t('dyn.unchecked') }));
      if (!merged.length) {
        host.innerHTML = '' + t('dyn.noHot') + '';
        return;
      }
      host.innerHTML = merged.map(function (s) {
        var status = s.installed
          ? (s.updateAvailable ? '<span class="badge warn">' + t('dyn.updateAvail', { from: escapeHtml(s.localVersion), to: escapeHtml(s.version) }) + '</span>' : '<span class="badge ok">' + t('dyn.installed') + '</span>')
          : '<span class="badge">' + t('dyn.notInstalled') + '</span>';
        var action = s.installed
          ? (s.updateAvailable
            ? '<button class="btn-amber" type="button" data-update-skill="' + escapeHtml(s.id) + '">' + t('dyn.update') + '</button>'
            : '')
          : '<button class="btn-amber" type="button" data-install-skill="' + escapeHtml(s.id) + '">' + t('dyn.download') + '</button>';
        return (
          '<div class="skill-card">' +
            '<div><b>' + escapeHtml(s.name) + '</b>' +
              '<div class="meta">' + escapeHtml(s.summary) + '</div>' +
              '<div class="path">' + escapeHtml(s.repo) + ' · v' + escapeHtml(s.version) + ' · ★' + s.stars + ' · ' + t('dyn.downloads', { n: s.downloads }) + '</div></div>' +
            '<div>' + status + '</div>' +
    ...
    setText('#skill-count', t('dyn.installedN', { n: list.length }));
    if (!list.length) {
      host.innerHTML = '' + t('dyn.noSkill') + '';
      return;
    }
    host.innerHTML = list.map(function (s) {
      return (
        '<div class="skill-card">' +
          '<div><b>' + escapeHtml(s.name) + '</b>' +
            '<div class="meta">' + escapeHtml(s.summary) + (s.note ? ' · ' + t('dyn.notePrefix') + escapeHtml(s.note) : '') + '</div>' +
            '<div class="path">' + escapeHtml(s.source) + ' · ' + escapeHtml(s.path) + ' · v' + escapeHtml(s.version) + '</div></div>' +
          starsHtml(s.id, s.rating) +
          '<div class="skill-actions">' +
            '<button class="btn-ghost" type="button" data-note-skill="' + escapeHtml(s.id) + '">' + t('dyn.note') + '</button>' +
            '<button class="btn" type="button" data-update-skill="' + escapeHtml(s.id) + '">' + t('dyn.update') + '</button>' +
            '<button class="btn" type="button" data-delete-skill="' + escapeHtml(s.id) + '">' + t('dyn.delete') + '</button>' +
          '</div>' +
        '</div>'
      );
    }).join('');
  ...
  function showHover(id, rect, anchor) {
    ...
    if (!(p.frameSrcs && p.frameSrcs.length) && global.FramespaceDesktop && global.FramespaceDesktop.loadProject) {
      global.FramespaceDesktop.loadProject(id, { silent: true }).then(function (detail) {
        var now = $("#hover-preview");
        if (!detail || !now || now.getAttribute("data-for") !== id || !now.classList.contains("is-on")) return;
        now.removeAttribute("data-for");
        showHover(id, rect, hoverAnchor);
      });
    }
  }
  ...
  function setCatalog(projects) {
    ...
    (projects || []).forEach(function (item) {
      var extra = item && item.id ? DETAILS[item.id] : null;
      CATALOG.push(extra ? Object.assign({}, item, extra) : item);
    });
    return CATALOG;
  }
  ...
  function renderJobs() {
    ...
    if (!list.length) {
      host.innerHTML = '' + t('dyn.noJobs') + '';
      return;
    }
    host.innerHTML = list.map(function (j) {
      var cls = 'job' + (j.status === 'error' ? ' is-error' : '') + (j.status === 'done' ? ' is-done' : '');
      var label = j.status === 'running' ? t('dyn.running') : (j.status === 'done' ? t('dyn.done') : (j.status === 'error' ? t('dyn.error') : j.status));
  ...
  function renderDoctor() {
    ...
    if (!cards.length) {
      host.innerHTML = '<div class="doc-card"><b>' + t('doctor.title') + '</b><div class="meta">' + t('dyn.doctorEmpty') + '</div></div>';
      return;
    }
  ...
  function openProject(id) {
    ...
    render();
    if (global.FramespaceDesktop && global.FramespaceDesktop.loadProject) {
      global.FramespaceDesktop.loadProject(id);
    }
  }
  ...
  function runCli(action) {
    ...
    render();
    toast(t('dyn.cmdPreview', { action: action }));
  }
  ...
  function bind() {
    var root = ui.root;
    var gridHost = $('#library-grid', root);
    if (gridHost) {
      gridHost.addEventListener('scroll', function () {
        if (ui.state && ui.state.view === 'library' && ui.state.layout !== 'list') renderLibrary();
      }, { passive: true });
    }
    var listHost = $('#library-list', root);
    if (listHost) {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('resize', function () {
        if (ui.state && ui.state.view === 'library') renderLibrary();
      });
    }
    root.addEventListener('click', function (ev) {
      ...
      if (renamePin) {
        ...
        if (typeof global.prompt === 'function') {
          var typed = global.prompt(t('dyn.aliasPrompt'), currentLabel);
          if (typed === null) return;
          nextName = typed;
        }
        ui.state.pinnedFolders = renamePinnedFolder(ui.state.pinnedFolders, renamePath, nextName);
        render();
        toast(t('dyn.alias', { name: '' }) +  pinnedFolderLabel({ path: renamePath, alias: String(nextName || '').trim() }));
        return;
      }
      ...
      if (pinF) {
        ...
        render();
        toast((willPin ? t('dyn.pinned', { name: folderDisplayName(pinPath) }) : t('dyn.unpinned', { name: folderDisplayName(pinPath) })));
        return;
      }
      ...
      if (addCol) {
        var name = t('dyn.newGroup');
        if (typeof global.prompt === 'function') {
          var typed = global.prompt(t('dyn.customName'), t('dyn.newGroup'));
          if (typed === null) return;
          name = typed;
        }
        ...
        render();
        toast(t('dyn.createdCustom', { name: '' }) +  (findCollection(added.collections, added.id) || {}).name);
        return;
      }
      ...
      if (noteBtn) {
        ...
        if (typeof global.prompt === 'function') {
          var nextNote = global.prompt(t('dyn.skillNote'), typed);
          if (nextNote === null) return;
          typed = nextNote;
        }
        ui.state.skills = annotateSkill(ui.state.skills, sid, typed);
        render();
        toast(t('dyn.noteSaved'));
        return;
      }
      ...
      if (delBtn) {
        ...
        render();
        toast(t('dyn.deletedMock', { name: ds.name }));
        return;
      }
      ...
      if (updBtn) {
        ...
        render();
        toast(t('dyn.updatedMock', { id: us.id }));
        return;
      }
      ...
      if (instBtn) {
        ...
        render();
        toast(t('dyn.downloadedMock', { id: hot.id }));
        return;
      }
        ...
        if (global.FramespaceDesktop && global.FramespaceDesktop.fetchCatalog) {
          global.FramespaceDesktop.fetchCatalog(url).then(function (payload) {
            try {
              ...
              render();
              toast(t('dyn.catalogOk'));
            } catch (err) {
              toast(err.message || t('dyn.catalogParse'));
            }
          }).catch(function (err) {
            ui.state.hotSkills = parseSkillCatalog(HOT_SKILL_CATALOG);
            ui.state.skillTab = 'hot';
            render();
            toast(t('dyn.catalogFallback', { err: err.message || err }));
          });
          return;
        }
        ui.state.hotSkills = parseSkillCatalog(HOT_SKILL_CATALOG);
      ...
      if (killAll) {
        ...
        render();
        toast(t('dyn.cleanedMock', { n: result.killed.length }));
        return;
      }
      ...
      if (killOne) {
        ...
        render();
        toast(t('dyn.killedMock', { pid: pid }));
        return;
      }
      ...
      if (t.closest('[data-preview-play]')) {
        var frame = document.getElementById('live-preview-frame');
        try {
          if (frame && frame.contentWindow) frame.contentWindow.postMessage('toggle', '*');
        } catch (_) {}
        return;
      }
    ...
    var localeSel = $('#set-locale', root);
    if (localeSel) {
      localeSel.addEventListener('change', function () {
        if (global.FramespaceI18n) global.FramespaceI18n.setLocale(localeSel.value);
        if (window.framespaceAPI) window.framespaceAPI.saveSettings({ locale: localeSel.value });
        applyI18n();
        if (window.framespaceAPI && window.framespaceAPI.doctor) {
          window.framespaceAPI.doctor(CATALOG).then(function (info) {
            applyDesktopState({ doctor: info });
          }).catch(function () {});
        }
        render();
      });
    ...
    root.addEventListener('click', function (ev) {
      var btn = ev.target && ev.target.closest && ev.target.closest('[data-set-locale]');
      if (!btn) return;
      var next = btn.getAttribute('data-set-locale');
      if (global.FramespaceI18n) global.FramespaceI18n.setLocale(next);
      if (window.framespaceAPI) window.framespaceAPI.saveSettings({ locale: next });
      applyI18n();
      if (window.framespaceAPI && window.framespaceAPI.doctor) {
        window.framespaceAPI.doctor(CATALOG).then(function (info) {
          applyDesktopState({ doctor: info });
    ...
    function hoverPointerStillInside() {
      if (typeof document === 'undefined' || typeof document.elementFromPoint !== 'function') return false;
      return pointerInHoverZone(document.elementFromPoint(lastPointer.x, lastPointer.y));
    }
    ...
    document.addEventListener('scroll', function () {
      ...
      if (!preview || !preview.classList.contains('is-on')) return;
      if (hoverPointerStillInside()) {
        cancelHoverTimers();
        if (hoverAnchor && hoverAnchor.getBoundingClientRect) {
          applyHoverPlacement(preview, hoverAnchor.getBoundingClientRect());
        }
        return;
      }
      cancelHoverTimers();
    ...
    root.addEventListener('drop', function (ev) {
      ...
      toast(t('dyn.dropped', { col: ((col && col.name) || colId), name: ((proj && proj.name) || projectId) }));
      if (global.FramespaceDesktop && global.FramespaceDesktop.persist) global.FramespaceDesktop.persist();
    });
